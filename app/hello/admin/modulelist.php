...
if(isset($_GET['delete'])){
    $id = (int)$_GET['delete'];
    $stmt = $mysqli->prepare("DELETE FROM modules WHERE id=?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    header("Location: " . PUBLIC_URL . "/index.php/admindash/modulelist");
    exit();
}

// Query modules with professors and assigned classes
$result = $mysqli->query("
...
<head>
<meta charset="UTF-8">
<title>Module List | macademia Faculty</title>
...
h2 { color: var(--primary); margin-bottom: 1.5rem; font-size: 1.5rem; font-weight: 700; }
table {
    width: 100%; border-collapse: collapse; background: var(--bg-card);
    backdrop-filter: var(--glass-blur); border: 1px solid var(--bg-card-border);
    border-radius: 20px; overflow: hidden;
}
th, td { padding: 1rem; border-bottom: 1px solid var(--bg-card-border); }
th { color: var(--primary); font-weight: 600; background: rgba(255, 255, 255, 0.06); }
td { color: var(--text-secondary); }
.delete-btn {
    color: var(--error); text-decoration: none; font-weight: 600; transition: var(--transition);
}
...
<div class="dashboard-wrapper">
    ...
    <aside class="sidebar" id="sidebar">
        <ul class="sidebar-menu">
            ...
            <li><a href="modulelist.php" class="sidebar-link active"><i class="bi bi-bookshelf"></i><span>Module List</span></a></li>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= htmlspecialchars($r['prof_nom'] . ' ' . $r['prof_prenom']) ?></td>
                        <td><?= htmlspecialchars($r['classes'] ?? 'None') ?></td>
                        <td>
                            <a href="?delete=<?= $r['id'] ?>" class="delete-btn" onclick="return confirmTripleDelete()">Delete</a>
                        </td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
