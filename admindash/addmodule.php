...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name)) {
        $msg = "<div style='color:#ff3b3b;margin-bottom:1rem;'>Module name required!</div>";
    } else {
        $stmt = $mysqli->prepare("INSERT INTO modules (module_name, professor_id) VALUES (?, ?)");
        $stmt->bind_param("si", $name, $professor_id);
        if($stmt->execute()){
            // Redirect to module list after successful addition
            header("Location: modulelist.php");
            exit();
        } else {
            $msg = "<div style='color:#ff3b3b;margin-bottom:1rem;'>Error adding module. Please try again.</div>";
        }
    }
}
?>
        ...
        <div class="form-box">
            ...
            <form method="POST">
                ...
                <select name="professor_id" required>
                    ...
                    <?php
                    $profs = $mysqli->query("SELECT id, nom, prenom FROM users WHERE role='professor' ORDER BY nom");
                    while($p = $profs->fetch_assoc()){
                        echo "<option value='{$p['id']}'>".htmlspecialchars($p['nom']." ".$p['prenom'])."</option>";
                    }
                    ?>
                </select>
                <button type="submit"><span class="btn-text">Add Module</span></button>
            </form>
        </div>
    </main>
</div>
