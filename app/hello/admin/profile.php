...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_photo'])) {
    $user = $mysqli->query("SELECT photo_path FROM users WHERE id = $admin_id")->fetch_assoc();
...
$user = $mysqli->query("SELECT nom, prenom, email, role, photo_path FROM users WHERE id = $admin_id")->fetch_assoc();
...
body { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; background: var(--bg-main); background-size: 400% 400%; animation: gradientShift 25s ease infinite; color: var(--text-primary); min-height: 100vh; overflow-x: hidden; }
@keyframes gradientShift { 0%, 100% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } }
.navbar { position: fixed; top: 0; left: 0; right: 0; z-index: 1000; height: 70px; background: var(--bg-panel); backdrop-filter: var(--glass-blur); border-bottom: 1px solid var(--bg-card-border); display: flex; align-items: center; padding: 0 2rem; justify-content: space-between; }
...
.notification-dropdown { position: absolute; top: 100%; right: 0; margin-top: 1rem; background: var(--bg-panel); border: 1px solid var(--bg-card-border); border-radius: 14px; min-width: 320px; box-shadow: var(--shadow); opacity: 0; visibility: hidden; transform: translateY(-10px); transition: var(--transition); z-index: 1001; }
...
<nav class="navbar">
    ...
    <div class="navbar-right">
        <button class="theme-toggle" id="themeToggle" title="Toggle theme"><i class="bi bi-moon-fill" id="themeIcon"></i></button>
...
<div class="dashboard-wrapper">
    <aside class="sidebar" id="sidebar">
        <ul class="sidebar-menu">
            <li><a href="<?php echo $public_url; ?>/index.php/admindash/profile" class="sidebar-link active"><i class="bi bi-person-circle"></i><span>Profile</span></a></li>
    ...
    <main class="main-content">
        <div class="page-header">
            ...
            <p class="page-subtitle">Manage your admin details and profile photo</p>
        </div>
        ...
        <div class="profile-grid">
            ...
            <div class="card">
                ...
                <div class="features">
                    <div class="feature-item"><i class="bi bi-people"></i> Manage users and approve requests faster.</div>
                    <div class="feature-item"><i class="bi bi-shield-lock"></i> Keep your admin account secure.</div>
